...
def calculateGamePower(selections):
    maxCubes = {}
    for selection in selections:
        for cube in selection.keys():
            if cube in maxCubes.keys():
                maxCubes[cube] = max(maxCubes[cube], selection[cube])
            else:
                maxCubes[cube] = selection[cube]
    product = 1
    for number in maxCubes.values():
        product *= number
    return product
...
for game in games.keys():
    sum += calculateGamePower(games[game])
print(sum)