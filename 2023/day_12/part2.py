...
def getPuzzleInput(fileLines):
    ...
    for fileLine in fileLines:
        conditions = re.findall('[#?]+', '?'.join([fileLine.split(' ')[0]] * 5))
        groups = list(map(int, re.findall('[0-9]+', ','.join([fileLine.split(' ')[1]] * 5))))
...
def printInputAnswer(input, answer):
    global count
    count += 1
    print(str(time.strftime("%Y-%m-%d %H:%M:%S")) + ' ' + str(count).rjust(4, ' ') + ' ' + '.'.join(input[0]).ljust(20, ' ') + ' - ' + ','.join(list(map(str, input[1]))).ljust(15, ' ') + ' - ' + str(answer))
    return answer
