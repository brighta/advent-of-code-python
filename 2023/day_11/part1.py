...
def expandRows(inputs, galaxies):
    ...
    for x in range(len(inputs[0])):
        ...
        xIndexes.append((0 if len(xIndexes) == 0 else xIndexes[-1]) + (2 if len(distinctValues) == 1 and distinctValues[0] == '.' else 1))
    yIndexes = []
    for y in range(len(inputs[0])):
        distinctValues = list(set(inputs[y]))
        yIndexes.append((0 if len(yIndexes) == 0 else yIndexes[-1]) + (2 if len(distinctValues) == 1 and distinctValues[0] == '.' else 1))
    newGalaxies = []
