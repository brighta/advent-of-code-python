...
cardOrders = ['J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A']
...
def getHandScore(hand):
    ...
    if distinctCards.count('J') > 0:
        distinctCards.remove('J')
    numberJacks = cards.count('J')
    # Five of a Kind
    if len(distinctCards) <= 1:
        return '7'
    # Four of a Kind
    if len(distinctCards) == 2 and (
            cards.count(distinctCards[0]) + numberJacks == 4 or
            cards.count(distinctCards[1]) + numberJacks == 4
    ):
        return '6'
    # Full House
    ...
    if len(distinctCards) == 3 and (
            cards.count(distinctCards[0]) + numberJacks == 3 or
            cards.count(distinctCards[1]) + numberJacks == 3 or
            cards.count(distinctCards[2]) + numberJacks == 3
    ):
        return '4'
    # Two Pair
