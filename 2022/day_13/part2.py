...
def isListInOrder(firstList, secondList):
    ...
    if type(firstList[0]) == type(secondList[0]) == type([]):
        ...
        if listIsInOrder == -1 or listIsInOrder == 1:
...
i = 0
while i < len(file):
    items.append(literal_eval(file[i].rstrip()))
    items.append(literal_eval(file[i + 1].rstrip()))
    i += 3
items.append([[2]])
items.append([[6]])
items = sorted(items, key=cmp_to_key(isListInOrder))
