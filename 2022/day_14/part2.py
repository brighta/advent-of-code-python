...
def addScanToGrid(xMin, xMax, yMin, yMax, grid, line):
    ...
    for i in range(len(lineParts)-1):
        ...
        xMax = newXMax
        if newYMax > yMax:
            grid.extend([["." for x in range(xMax - xMin + 1)] for y in range(newYMax - yMax)])
            yMax = newYMax
        if firstX == secondX:
...
def addFloorToGrid(xMin, xMax, yMin, yMax, grid):
    newXMin = min(xMin, 500 - len(grid) - 4)
    newXMax = max(xMax, 500 + len(grid) + 4)
    newGrid = []
    for row in grid:
        if newXMin < xMin:
            row[:0] = ["." for x in range(xMin - newXMin)]
    ...
    xMax = newXMax
    yMax = yMax + 1
    grid.append(["." for x in range(xMax - xMin + 1)])
    return xMin, xMax, yMin, yMax, grid
...
def dropSand(grid, sandX):
    ...
    while (sandX > 0 and sandX < len(grid[0])-1 and sandY < len(grid)-1 and not sandStopped):
        if grid[sandY+1][sandX] == ".":
            ...
            sandStopped = True
    grid[sandY][sandX] = "o"
    sandNoLongerMoving = sandX == stopPointX and sandY == 0
    return grid, sandNoLongerMoving
...
xMin, xMax, yMin, yMax, grid = addFloorToGrid(xMin, xMax, yMin, yMax, grid)
sandNoLongerMoving = False
unitsOfSand = 0
while not sandNoLongerMoving:
    grid, sandNoLongerMoving = dropSand(grid, 500 - xMin)
    unitsOfSand += 1
print(unitsOfSand)
